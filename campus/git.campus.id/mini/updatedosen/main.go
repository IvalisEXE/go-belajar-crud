...
import (
	...
	ep "MCCampus/campus/git.campus.id/mini/updatedosen/endpoint"
	pb "MCCampus/campus/git.campus.id/mini/updatedosen/grpc"
	svc "MCCampus/campus/git.campus.id/mini/updatedosen/server"

	cfg "MCCampus/campus/git.campus.id/mini/util/config"
	run "MCCampus/campus/git.campus.id/mini/util/grpc"
...
func main() {
	...
	var server pb.DosenServiceServer
	{
		...
		service := svc.NewDosen(dbReadWriter)
		endpoint := ep.NewDosenEndpoint(service)
		fmt.Println(endpoint)
		server = ep.NewGRPCDosenServer(endpoint, tracer, logger)
	}
	...
	pb.RegisterDosenServiceServer(grpcServer, server)
